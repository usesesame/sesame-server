...
func New(config Config) http.Handler {
	...
	// Public metadata: published facts only, no session or vault data.
	...
	// Linked desktop clients: token-authenticated, never browser reachable.
	...
	// The release pipeline authenticates by bearer credential alone.
	...
	// The catch-all stays outside the registry: an unregistered request must
	// resolve to it (404) or to a known path's Allow set (405), never to a policy.
