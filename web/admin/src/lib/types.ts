...
export type Release = { id: string; channel: string; platform: string; architecture: string; version: string; url: string; sha256: string; signature: string; signingKeyId: string; supportedWindows: string; releaseNotesUrl: string; rollbackNotice: string; status: 'draft' | 'published' | 'withdrawn'; rolloutPercent: number; updateEnabled: boolean; killSwitch: boolean; manifestRevision: number; publicationBlockers: string[]; audit: AuditEntry[]; publishedAt?: string; artifact?: ReleaseArtifact }
